...
import {
  getModalMouseNavigationIntent,
  isMouseNavigationButton,
  shouldHandleEnterAsWizardAdvance,
...
export const catalogModeOptions: Array<{
  label: string;
...
export function canSaveNotionSynchronization(
  mode: AppSettings["catalogSourceMode"],
  testResult: NotionCatalogConnectionTestResult | null,
): boolean {
  return mode === "manual" || Boolean(testResult?.ok);
}

export function createNotionCatalogConnectionTestErrorResult(
  error: unknown,
): NotionCatalogConnectionTestResult {
  ...
  return {
    ok: false,
    message: "Notion catalog connection test failed.",
    title: null,
    extractedBlockCount: 0,
  };
}

export function canSaveNotionTokenDraft(
  tokenDraft: string,
  tokenDraftTestStatus: NotionTokenDraftTestStatus,
  isSavingToken: boolean,
): boolean {
  return (
    Boolean(tokenDraft.trim()) &&
    tokenDraftTestStatus === "success" &&
    !isSavingToken
  );
}
...
export async function testNotionCatalogSource({
  ...
  if (mode === "notion") {
    return onTestNotionCatalogConnection(sourceUrl.trim(), token);
  }

  await onChangeCatalogSettings({
...
export function NotionSynchronizationGuide({
  ...
  const [mode, setMode] = useState<AppSettings["catalogSourceMode"]>(
    settings.catalogSourceMode === "public-exportable"
      ? "notion"
      : settings.catalogSourceMode,
  );
  ...
  const [tokenDraftTestStatus, setTokenDraftTestStatus] =
    useState<NotionTokenDraftTestStatus>("idle");
  const [testResult, setTestResult] =
    useState<NotionCatalogConnectionTestResult | null>(null);
  const [isSavingToken, setIsSavingToken] = useState(false);
  const [isTesting, setIsTesting] = useState(false);
  const visibleSteps =
    mode === "manual"
      ? notionSteps.filter((candidate) => candidate.id === "source")
      : notionSteps;
  ...
  const canContinue =
    ...
    step === "review";
  const canSaveSynchronization =
    canSaveNotionSynchronization(mode, testResult) &&
    (mode !== "notion" || hasToken);
  const canSaveTokenDraft = canSaveNotionTokenDraft(
    tokenDraft,
    tokenDraftTestStatus,
    isSavingToken,
  );
  const canTestSource = canTestNotionCatalogSource(
    mode,
    sourceUrl,
    hasToken,
    tokenDraft,
  ...
  const sourceLabel = useMemo(
    () =>
      catalogModeOptions.find((option) => option.value === mode)?.label ??
      "Unknown",
    [mode],
  );
  ...
  async function testSource() {
    ...
    try {
      const result = await runNotionCatalogSourceTest({
        mode,
        sourceUrl,
      ...
      if (tokenDraft.trim()) {
        ...
        setTokenMessage(
          result.ok
            ? "This token passed Test source and can be saved."
            : "Test source failed. The draft token was not saved.",
        );
      }
    } catch (error) {
  ...
  async function saveAndClose() {
    ...
    await onChangeCatalogSettings({
      catalogSourceMode: mode,
      catalogSourceUrl: mode === "manual" ? "" : sourceUrl.trim(),
    });
    onClose();
  }
  ...
  function handleModalMouseUp(event: ReactMouseEvent<HTMLElement>) {
    const intent = getModalMouseNavigationIntent(event.button, {
      ...
      canGoForward:
        step === "review"
          ? canContinue && !isTesting && canSaveSynchronization
          : canContinue,
    });
    if (!intent) return;
    event.preventDefault();
  ...
  return (
    ...
    >
      ...
      >
        <PanelHeader
          title="Set Catalog Source"
          subtitle="Choose Manual catalog for local Areas or connect the JTF Sync Catalog page for official area sync."
          onClose={onClose}
        />
        <div className="border-b border-[#dfe1e6] bg-[#f7f8fa] px-5 py-3">
          <div
            className={`grid gap-2 ${visibleSteps.length === 1 ? "grid-cols-1" : "grid-cols-3"}`}
          >
        ...
        <div className="min-h-[360px] flex-1 overflow-y-auto p-6">
          {step === "source" ? (
            <GuideSection
              ...
              description="Choose whether Areas are managed locally or synced from the Notion catalog. Manual catalog saves immediately and skips external setup."
            >
              <SourceModeSelect
                label="Catalog mode"
                value={mode}
              ...
              {mode !== "manual" ? (
                ...
                <FeedbackNote variant="warning">
                  Manual catalog keeps Areas editable in Categories. No Notion
                  token, page URL, or connection test is needed.
                </FeedbackNote>
              )}
            </GuideSection>
          ) : null}
          {step === "token" ? (
            <GuideSection
              ...
              description="Create or copy the internal integration secret, test it against the selected page, then save it. The token stays in the OS credential store."
            >
                ...
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div className="min-w-0">
                    <div className="text-sm font-semibold text-[#172b4d]">
                      {hasToken ? "Credential saved" : "No credential saved"}
                    </div>
                    <p className="text-xs leading-relaxed text-[#6b778c]">
                      The Notion page must also be shared with the Jira Task
                      Forge connection.
                    </p>
                  </div>
                  <Button
              ...
              <GuideInput
                ...
                placeholder={
                  hasToken
                    ...
                    : "Paste Notion integration token"
                }
                secret
                value={tokenDraft}
                onChange={(value) => {
              ...
              {tokenMessage ? (
                <FeedbackNote className="mt-4" variant="success">
                  {tokenMessage}
                </FeedbackNote>
              ) : null}
            </GuideSection>
          ...
          {step === "review" ? (
            <GuideSection
              ...
              description="Save the source settings, then test that the app can read the JTF catalog contract."
            >
              <ReviewRows
                rows={[
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                ...
                <Button
                  className="settings-button-primary"
                  disabled={isTesting || !canSaveSynchronization}
                  icon={<Check size={14} />}
        ...
        <div className="flex items-center justify-between border-t border-[#dfe1e6] bg-[#f7f8fa] px-5 py-3">
          ...
          {step === "review" ? (
            <Button
              className="settings-button-primary"
              disabled={!canContinue || isTesting || !canSaveSynchronization}
              icon={<Check size={14} />}
              onClick={saveAndClose}
            >
              Done
...
function GuideSection({
  ...
  return (
    <section>
      ...
      <p className="mb-4 text-sm leading-relaxed text-[#6b778c]">
        {description}
      </p>
      {children}
    </section>
...
function GuideInput({
  ...
  return (
    <label className="mb-3 block">
      <span className="mb-1 block text-xs font-medium text-[#6b778c]">
        {label}
      </span>
      <input
...
function SourceModeSelect({
  ...
  return (
    <div className="relative mb-3">
      <div className="mb-2 block text-xs font-medium text-[#6b778c]">
        {label}
      </div>
      <button
